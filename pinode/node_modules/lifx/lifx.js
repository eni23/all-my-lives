...
Lifx.prototype._setupPacketListener = function () {
    ...
    this.on('rawpacket', function (pkt, rinfo) {
        var bulb, found = false, i;
				...
				//console.log("incoming package, type: "+pkt.packetTypeShortName+" sender: "+rinfo.address);
...
// v2 by eni, sends packet to gw addr and not bulb in rcv
		...
		if (Buffer.isBuffer(bulb)) {
			...
			if (b) {
				...
				_(this.gateways).each(function(gw, ip) {
					if (gw.bulbAddress == bulb){
						bulbAddress=new Buffer(bulb,"hex");
					}
				});
				if (!bulbAddress){
					//console.log(bulb + ": not a valid bulb");
					return;
				}
			}
		}
...
Lifx.prototype._sendToOneOrAll_orig = function(command, bulb) {
	var self = this;

	var gw = this.gateways["10.10.0.163"];
